...
export default function DiscountProductDetailsClient({
  ...
  const { originalPrice, discountedPrice, discountAmount, discountPercentage } =
    priceInfo;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        ...
        <Link
          ...
          className="bg-teal-600 text-white px-6 py-3 diva-rounded hover:bg-teal-700 transition-colors"
        >
    ...
    <div className="bg-white py-4">
      <Container>
        ...
        <nav className="text-xs md:text-sm text-gray-600 mb-2 flex items-center gap-1.5 md:gap-2 flex-nowrap overflow-hidden">
          <Link href="/" className="hover:text-teal-600 shrink-0">
            Home
          </Link>
          <span className="shrink-0">/</span>
          <Link
            href="/discount-campaigns"
            className="hover:text-teal-600 shrink-0 whitespace-nowrap"
          >
            Discount Campaigns
          </Link>
          <span className="shrink-0">/</span>
        ...
        {hasCampaignDiscount && campaignInfo && (
          <div className="mb-3 py-2 px-3 sm:py-2.5 sm:px-4 rounded-[12px] bg-[#5A0C3D]/5 border border-[#5A0C3D]/15 font-outfit shadow-xs">
            <div className="flex flex-row items-center justify-between gap-2 sm:gap-4">
              {/* Left Side Info: Stacked vertically on mobile, row on desktop */}
              <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3 min-w-0">
                <div className="text-white text-xs md:text-sm font-bold px-2.5 py-1 sm:py-1.5 sm:px-3 rounded-lg shadow-xs bg-[#5A0C3D] uppercase tracking-wider shrink-0 self-start sm:self-auto">
                  {getBadgeText()}
                </div>
                <div className="min-w-0">
                  <h3 className="font-bold text-gray-900 text-sm md:text-lg leading-tight truncate">
                  ...
                  <p className="text-[11px] md:text-sm text-gray-600 font-light mt-0.5 truncate">
                    ...
                    {campaignInfo.maxDiscountAmount &&
                      ` (max ৳${formatPrice(campaignInfo.maxDiscountAmount)})`}
                  </p>
                </div>
              </div>
              ...
              {campaignInfo.endAt && campaignInfo.showCountdown && (
                <div className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2.5 bg-white/80 backdrop-blur-xs p-[5px] sm:px-2 sm:py-1.5 rounded-lg border border-[#5A0C3D]/10 shadow-2xs shrink-0 self-end sm:self-auto">
                  <span className="text-[9px] md:text-[11px] font-bold text-[#5A0C3D] uppercase tracking-widest hidden sm:inline">
                    Offer ends in:
                  </span>
                    ...
                    <div className="flex-1">
                      ...
                      {campaignInfo.endAt && (
                        <div className="flex items-center space-x-1.5 mt-1.5 text-xs">
                          <span>⏰</span>
                          <span className="font-semibold text-[#5A0C3D]">
                            Limited time offer!
                          </span>
                          <span className="text-gray-500 font-light">
                            Ends{" "}
                            {new Date(campaignInfo.endAt).toLocaleDateString()}
                          </span>
                        </div>
                      )}
                    </div>
